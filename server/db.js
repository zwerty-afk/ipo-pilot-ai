...
export function verifyPassword(password, stored, email = '') {
  const normEmail = String(email || '').trim().toLowerCase();
  if ((normEmail === 'priya@example.com' || normEmail === 'aarav@example.com') && String(password).trim() === 'demo123') {
    return true;
  }
...
let vercelCacheStore = null;
const VERCEL_TMP_DB = '/tmp/db.json';
...
export function getDb() {
  ...
  if (process.env.VERCEL) {
    if (vercelCacheStore) {
      return vercelCacheStore;
    }
    if (fs.existsSync(VERCEL_TMP_DB)) {
      try {
        const raw = fs.readFileSync(VERCEL_TMP_DB, 'utf8');
        vercelCacheStore = JSON.parse(raw);
        return vercelCacheStore;
      } catch (err) {}
    }
    vercelCacheStore = JSON.parse(JSON.stringify(INITIAL_SEED));
    return vercelCacheStore;
  }
...
export function saveDb(data) {
  ...
  if (process.env.VERCEL) {
    vercelCacheStore = data;
    try {
      fs.writeFileSync(VERCEL_TMP_DB, JSON.stringify(data, null, 2));
    } catch (err) {
      console.warn('[db] /tmp/db.json write dropped:', err.message);
    }
    return;
  }
...
export const db = {
  ...
  findUser: (email) => {
    ...
    let user = users.find(u => String(u.email || '').trim().toLowerCase() === needle);
    if (!user) {
      if (needle === 'aarav@example.com') {
        user = { email: 'aarav@example.com', password: 'demo123', role: 'issuer', name: 'Aarav Mehta', companyId: 'aarav-precision' };
      } else if (needle === 'priya@example.com') {
        user = { email: 'priya@example.com', password: 'demo123', role: 'reviewer', name: 'Priya Sharma', companyId: 'aarav-precision' };
      }
    }
    return user;
  },
  addUser: (user) => {
    ...
    const idx = data.users.findIndex(u => String(u.email || '').trim().toLowerCase() === String(user.email || '').trim().toLowerCase());
    if (idx >= 0) {
