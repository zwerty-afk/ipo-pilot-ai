...
import { SECTION_KEYS, getIntakeForSection } from './ChapterHealthSidebar';
...
export default function DraftCanvas({ showToolbar = true, mode = 'chapter' }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const handleOpenSourceDrawer = (e) => {
      setShowSourceDrawer(true);
      if (e.detail?.sourceRef) {
        handleSourceClick(e.detail.sourceRef);
      }
    };
    window.addEventListener('open-drhp-source-drawer', handleOpenSourceDrawer);
    return () => window.removeEventListener('open-drhp-source-drawer', handleOpenSourceDrawer);
  }, []);
  ...
  return (
    <div className="space-y-4 font-sans relative" ref={documentCanvasRef}>
      ...
      {!showToolbar ? (
        /* Reviewer Workspace Center Column: Full-width document card matching Word/Docs */
        <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-8 font-serif text-slate-900 min-h-[75vh] relative">
        ...
        /* Draft Prospectus Page Canvas View */
